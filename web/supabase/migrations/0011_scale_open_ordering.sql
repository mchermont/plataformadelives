-- 0011: Fase E.2 — escalas, respostas abertas e ranking (ordering)
-- Novos tipos sobre o mesmo motor de atividades (0009/0010).

alter table activities drop constraint activities_type_check;
alter table activities add constraint activities_type_check
  check (type in ('word_cloud', 'poll', 'quiz', 'quiz_ranking',
                  'scale', 'open_text', 'ordering'));

-- ============================================================
-- submit_activity_response: novos tipos
--   scale:    payload {"ratings": [3,5,...]} (1 valor por afirmação)
--   open_text:payload {"text": "..."} (blocklist + moderação prévia)
--   ordering: payload {"order": [2,0,1,...]} (permutação dos índices)
-- ============================================================
...
-- ============================================================
-- activity_results: agregados dos novos tipos
-- ============================================================
...
begin
  ...
  if v_activity.type = 'word_cloud' then
    ...
    select count(*) into v_total
      from activity_responses where activity_id = p_activity_id and approved;
    -- posição média de cada item (1 = topo); menor média = mais bem ranqueado
